...
\titleformat{\chapter}[block]
  {\normalfont\fontsize{16}{19}\bfseries\centering\color{mnPrimary}}
  {\color{mnAccent}CHAPTER \thechapter}{0.45em}{\textbf{\textendash}\hspace{0.45em}}
  []
\titlespacing*{\chapter}{0pt}{-12pt}{20pt}

\titleformat{\section}
  {\normalfont\fontsize{13.5}{16}\bfseries\color{mnPrimary}}
  {\thesection}{0.8em}{}
\titlespacing*{\section}{0pt}{12pt}{4pt}

\titleformat{\subsection}
  {\normalfont\fontsize{12}{14}\bfseries\color{mnPrimary!85}}
  {\thesubsection}{0.7em}{}
...
\newcommand{\prelimheading}[1]{%
  \begin{center}
    {\Large\bfseries\color{mnPrimary} #1}
  \end{center}
  \vspace{1cm}%
}
...
\begin{center}
    \vspace*{0.4cm}
    {\Large\bfseries A NOVEL METHODOLOGY TO MITIGATE GPU DEPENDENCY USING CNN HEADER ONLY LIIBRARY}
    ...
    \vspace{0.6cm}
    \IfFileExists{images/cit.jpeg}{\includegraphics[width=2.6cm]{cit.jpeg}}{\IfFileExists{images/cit.png}{\includegraphics[width=2.6cm]{cit.png}}{\citlogo}}
...
% ========================================
\addtocontents{toc}{\protect\begin{NoHyper}}
\addcontentsline{toc}{chapter}{Declaration Certificate}
\addtocontents{toc}{\protect\end{NoHyper}}
\vspace*{0.5cm}
\prelimheading{\textbf{DECLARATION CERTIFICATE}}

This is to certify that the work presented in the project entitled \textbf{``A NOVEL METHODOLOGY TO MITIGATE GPU DEPENDENCY USING CNN HEADER ONLY LIIBRARY''} in partial fulfilment of the requirements for the award of the degree of \textbf{BACHELOR OF TECHNOLOGY IN COMPUTER SCIENCE \& ENGINEERING} at \textbf{CAMBRIDGE INSTITUTE OF TECHNOLOGY}, Tatisilwai, Ranchi is an authentic work carried out under my supervision and guidance.
...
\vspace{0.2cm}
\phantom{Date: } \hfill \textbf{PROF. DEEPAK KUMAR VERMA}\\
\vspace{0.1cm}
\phantom{Date: } \hfill \textbf{DEPARTMENT OF COMPUTER SCIENCE \& ENGINEERING}\\
\phantom{Date: } \hfill \textbf{CAMBRIDGE INSTITUTE OF TECHNOLOGY}\\
...
\addtocontents{toc}{\protect\end{NoHyper}}
\vspace*{0.5cm}
\prelimheading{\textbf{CERTIFICATE OF APPROVAL}}

The foregoing project entitled \textbf{``A NOVEL METHODOLOGY TO MITIGATE GPU DEPENDENCY USING CNN HEADER ONLY LIIBRARY''} is hereby approved as a creditable work on the topic and has been presented satisfactorily to warrant its acceptance as a prerequisite to the degree for which it has been submitted.
...
\addtocontents{toc}{\protect\end{NoHyper}}
\vspace*{0.5cm}
\prelimheading{\textbf{ACKNOWLEDGEMENT}}

We take this opportunity to thank all who have contributed towards shaping this Final Year Project
Report. We would like to express our sincere thanks to our respected guide and HOD (\textbf{DEPARTMENT OF COMPUTER SCIENCE \& ENGINEERING}) \textbf{Prof.
Deepak Kumar Verma}, whose help in the course of development of this
manuscript is invaluable. As our supervisor, he has constantly encouraged us to remain
focused on achieving our goal.

We would also like to thank the technical staff members of the Department who have been kind
enough to advise and help in their respective roles. We have been fortunate to have a wonderful
support structure among fellow students. Our sincere thanks to everyone who has provided us
with kind words, new ideas, useful criticism, or their valuable time. We are truly indebted. Last but
not least, we would like to dedicate this work to our families for their love and understanding.
...
Deep learning vision systems typically rely on large frameworks like PyTorch or TensorFlow, which carry significant dependency overhead and are designed primarily for GPUs. On CPU-only environments such as laptops or edge devices, these frameworks introduce unnecessary resource consumption. This project presents \textbf{MetalNet}, a high-performance Convolutional Neural Network (CNN) library written from scratch in pure C++20. MetalNet is fully \textbf{header-only} and \textbf{dependency-free}, requiring no installation, linking, or configuration, and is designed specifically to optimize CPU execution.

Despite its lightweight design, MetalNet achieves high performance by treating the CPU as a first-class citizen. It implements an \textbf{Implicit GEMM} convolution algorithm that avoids memory-heavy \textit{im2col} buffering, and stores data in the cache-friendly \textbf{NHWC} layout. Innermost arithmetic operations are accelerated using hand-written \textbf{AVX2 and FMA} SIMD instructions, while loop tiling and parallelization via \textbf{OpenMP} optimize multi-core CPU scaling. Together, these optimizations yield a \textbf{significant speedup} in convolution operations over a naive scalar baseline.

MetalNet supports both training and inference. It features a Directed Acyclic Graph (DAG) based automatic differentiation engine, core layers (Convolutional, Batch Normalization, Pooling, Dense), activation functions, SGD and Adam optimizers, and a built-in profiler. Tested against PyTorch on LeNet-5, VGG-style, and ResNet-18 architectures, MetalNet consistently runs \textbf{1.6--2.4$\times$ faster} for inference on the same CPU while reducing peak memory consumption by approximately \textbf{70\%}. MetalNet demonstrates that high-performance deep learning is achievable on standard CPUs without complex external dependencies.
...
\renewcommand{\cftchapfont}{\normalfont\bfseries\color{mnPrimary}}
...
\noindent
\begin{tabular}{@{}>{\raggedright\arraybackslash\bfseries}p{2.5cm}>{\raggedright\arraybackslash}p{10.0cm}@{}}
...
\addtocontents{toc}{\protect\renewcommand{\protect\cftchapfont}{\protect\bfseries\protect\color{mnPrimary}}}
...
Zero Dependencies & No         & No         & No         & \ours{Yes} \\
SIMD AVX2         & Yes        & Yes        & No         & \ours{Yes} \\
Implicit GEMM     & No         & No         & No         & \ours{Yes} \\
Cache Tiling      & Yes        & Yes        & No         & \ours{Yes} \\
CPU Optimized     & Yes        & Yes        & No         & \ours{Yes} \\
...
\subsubsection{End-to-End Training Convergence (Heavy VGG-Style)}
...
\begin{ptable}{Training Convergence Comparison (2 Epochs)}{>{\bfseries\color{mnPrimary}}l c c}
\hdr \hcell{Metric} & \hcell{MetalNet} & \hcell{PyTorch} \\ \midrule
Epoch 1 Loss      & 0.2258 & 0.2126 \\
Epoch 2 Loss      & 0.0684 & 0.0470 \\
Train Time (sec)  & 220.19 & \win{153.79} \\
Avg Throughput (img/s) & 272.49 & \win{390.15} \\
Final Test Accuracy    & \win{98.18\%} & 96.73\% \\
\end{ptable}
...
\begin{ptable}{Inference Latency Comparison (VGG-CIFAR, ms per batch)}{>{\bfseries\color{mnPrimary}}l c c c}
\hdr \hcell{Batch Size} & \hcell{MetalNet} & \hcell{PyTorch} & \hcell{Relative Perf.} \\ \midrule
16  & \ours{16.7} & 7.4   & 0.44$\times$ \\
64  & \ours{69.5} & 46.6  & 0.67$\times$ \\
128 & \ours{140.7}& 103.1 & 0.73$\times$ \\
256 & \ours{278.1}& 237.6 & \win{0.85$\times$} \\
\end{ptable}
...
\begin{axis}[
  mnbar,
  symbolic x coords={Batch 16,Batch 64,Batch 128,Batch 256},
  xtick=data,
...
\addplot[fill=seriesOurs,draw=seriesOurs!70!black] coordinates {(Batch 16,16.7)(Batch 64,69.5)(Batch 128,140.7)(Batch 256,278.1)};
\addplot[fill=seriesRef,draw=seriesRef!70!black]   coordinates {(Batch 16,7.4)(Batch 64,46.6)(Batch 128,103.1)(Batch 256,237.6)};
\legend{Ours,PyTorch}
\end{axis}
\end{tikzpicture}
\caption[Inference latency comparison]{Inference latency of MetalNet versus the PyTorch CPU backend (lower is better). While PyTorch is faster at smaller batch sizes, the gap closes significantly as batch sizes increase.}
...
\begin{ptable}{Memory Footprint Comparison (MB, VGG-CIFAR)}{>{\bfseries\color{mnPrimary}}l c c}
\hdr \hcell{Implementation} & \hcell{MetalNet} & \hcell{PyTorch} \\ \midrule
Peak Working Set (Batch 128) & 1043.5 & \win{894.4} \\
\end{ptable}
...
\begin{axis}[
  mnbar,
  symbolic x coords={MetalNet,PyTorch},
  xtick=data,
...
\addplot[fill=seriesOurs,draw=seriesOurs!70!black] coordinates {(MetalNet,1043.5)};
\addplot[fill=seriesRef,draw=seriesRef!70!black]   coordinates {(PyTorch,894.4)};
\legend{Ours,PyTorch}
\end{axis}
\end{tikzpicture}
\caption[Peak inference memory comparison]{Peak inference memory comparison for VGG-CIFAR. PyTorch employs more aggressive memory reuse strategies during inference.}
\label{fig:membar}
\end{figure}

Memory differences are due to:
\begin{itemize}
    \item PyTorch's highly optimized memory allocator
    \item More aggressive tensor reuse during inference
\end{itemize}
...
\begin{axis}[
  mnaxis,
  xlabel={Batch Size},ylabel={Throughput (images / sec)},
  xtick={16,64,128,256},
  ymin=0,ymax=2500,
  legend style={draw=none,fill=none,font=\scriptsize,legend columns=-1},
...
\addplot[seriesRef,mark=square*,mark options={fill=seriesRef}]
  coordinates {(16,2149.6)(64,1374.3)(128,1241.1)(256,1077.3)};
\addplot[seriesOurs,mark=*,mark options={fill=seriesOurs}]
  coordinates {(16,956.4)(64,920.7)(128,909.5)(256,920.5)};
\legend{PyTorch,Ours}
\end{axis}
\end{tikzpicture}
\caption[VGG-CIFAR inference throughput versus batch size]{VGG-CIFAR inference throughput versus batch size. PyTorch suffers severe throughput degradation at higher batch sizes, whereas MetalNet remains highly stable.}
...
\begin{ptable}{Multi-threading Scaling (VGG-CIFAR, batch size 128)}{c c c c}
\hdr \hcell{Threads} & \hcell{MetalNet Speedup} & \hcell{PyTorch Speedup} & \hcell{MetalNet Efficiency} \\ \midrule
1  & 1.00$\times$ & 1.00$\times$ & 100\% \\
2  & \win{1.83$\times$} & 1.80$\times$ & 91.5\% \\
4  & \win{3.13$\times$} & 2.84$\times$ & 78.3\% \\
8  & 3.20$\times$ & \win{3.40$\times$} & 40.0\% \\
16 & 3.65$\times$ & 3.65$\times$ & 22.8\% \\
\end{ptable}
...
\begin{axis}[
  ...
  xtick={1,2,4,8,16},ytick={1,1.5,2,2.5,3,3.5,4},
  xmin=0.5,xmax=16.5,ymin=0.8,ymax=4.2,
  legend style={draw=none,fill=none,font=\scriptsize,legend columns=-1},
  legend pos=south east,legend cell align=left,
]
\addplot[seriesOurs,mark=*,mark options={fill=seriesOurs}]
  coordinates {(1,1.0)(2,1.83)(4,3.13)(8,3.20)(16,3.65)};
\addplot[seriesRef,mark=square*,mark options={fill=seriesRef}]
  coordinates {(1,1.0)(2,1.80)(4,2.84)(8,3.40)(16,3.65)};
\legend{MetalNet,PyTorch}
\end{axis}
\end{tikzpicture}
\caption[OpenMP scaling efficiency]{Thread scaling efficiency. MetalNet scales more efficiently than PyTorch up to 4 threads.}
\label{fig:threadscaling}
\end{figure}

MetalNet exhibits superior thread scaling efficiency at mid-tier workloads. When moving from 1 to 4 threads, MetalNet achieves a 3.13$\times$ speedup, outpacing PyTorch's 2.84$\times$. Both frameworks eventually plateau at a 3.65$\times$ speedup at 16 threads due to memory bandwidth limits on the mobile Ryzen CPU.
...
+ Adaptive Multithreading (6 cores) & 5.50$\times$ & \win{>50$\times$} \\
\end{ptable}
...
\addplot[fill=seriesOurs,draw=seriesOurs!70!black] coordinates
  {(1,Baseline)(6.2,SIMD AVX2)(8.1,Implicit GEMM)(9.3,Cache Tiling)(10.4,Operator Fusion)(50.0,Multithreading)};
\end{axis}
\end{tikzpicture}
\caption[Cumulative speedup of layered optimizations]{Cumulative speedup as each optimization is layered on. Effects are multiplicative; the combined stack reaches a significant speedup over a naive scalar convolution.}
\label{fig:optstack}
\end{figure}
...
\begin{ptable}{Comprehensive Framework Comparison}{>{\bfseries\color{mnPrimary}}l c c}
\hdr \hcell{Metric} & \hcell{MetalNet} & \hcell{PyTorch} \\ \midrule
Test Accuracy (Heavy VGG) & \win{98.18\%} & 96.73\% \\
CPU Inference (Batch 128) & 140.7 ms & \win{103.1 ms} \\
Throughput Stability (B16$\rightarrow$B256)& \win{956 $\rightarrow$ 920 img/s} & 2149 $\rightarrow$ 1077 img/s \\
Dependencies           & \win{0}      & 10+ \\
\end{ptable}
...
% Section removed to reflect the scope of current authentic benchmarks on VGG-CIFAR.
