...
Despite its lightweight design, MetalNet achieves high performance by treating the CPU as a first-class citizen. It implements an \textbf{Implicit GEMM} convolution algorithm that avoids memory-heavy \textit{im2col} buffering, and stores data in the cache-friendly \textbf{NHWC} layout. Innermost arithmetic operations are accelerated using hand-written \textbf{AVX2 and FMA} SIMD instructions, while loop tiling and parallelization via \textbf{OpenMP} optimize multi-core CPU scaling. Together, these optimizations yield a \textbf{57$\times$ speedup} in convolution operations over a naive scalar baseline.
